  ...
  return (
    <div class="space-y-6">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Prioritized support and onboarding attention for beta customers.</p>
      </div>
            ...
            {(item) => (
              <li class="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                <div>
                  ...
                  <p class="text-xs uppercase tracking-wide text-slate-400">{item.kind.replace("_", " ")}</p>
                </div>
                <Show when={item.customer_id}>
                  <A
                    href={`/app/platform-command/customers/${item.customer_id}`}
                    class="rounded-md border border-slate-200 px-2.5 py-1 text-xs font-medium hover:bg-slate-50"
                  >
                    Open customer
                  </A>
                </Show>
...
function Stat(props: { label: string; value?: number; href: string }) {
  return (
    <A href={props.href} class="rounded-xl border border-slate-200 bg-white px-4 py-3 hover:border-slate-300">
