...
type Props = {
  value: () => PurchaseRequestStatusFilters;
  onChange: (next: PurchaseRequestStatusFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};
...
export function PurchaseRequestStatusFilter(props: Props) {
  ...
  const patch = (p: Partial<PurchaseRequestStatusFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-text-primary">Purchase Request Status</h2>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Domestic / Foreign">
          <div class="flex flex-wrap gap-4 text-sm">
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_domestic_foreign" checked={(props.value().domestic_foreign ?? "all") === "all"} onChange={() => patch({ domestic_foreign: "all" })} />
              All
            </label>
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_domestic_foreign" checked={props.value().domestic_foreign === "domestic"} onChange={() => patch({ domestic_foreign: "domestic" })} />
              Domestic
            </label>
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_domestic_foreign" checked={props.value().domestic_foreign === "foreign"} onChange={() => patch({ domestic_foreign: "foreign" })} />
              Foreign
            </label>
          </div>
        </Field>
        <Field label="Send status">
          <div class="flex flex-wrap gap-4 text-sm">
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_send_status" checked={(props.value().send_status ?? "all") === "all"} onChange={() => patch({ send_status: "all" })} />
              All
            </label>
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_send_status" checked={props.value().send_status === "unsent"} onChange={() => patch({ send_status: "unsent" })} />
              Unsent
            </label>
            <label class="flex items-center gap-1.5">
              <input type="radio" name="status_send_status" checked={props.value().send_status === "sent"} onChange={() => patch({ send_status: "sent" })} />
              Sent
            </label>
          </div>
        </Field>
