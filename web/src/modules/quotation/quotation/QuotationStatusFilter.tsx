...
type Props = {
  value: () => QuotationStatusFilters;
  onChange: (next: QuotationStatusFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};
...
export function QuotationStatusFilter(props: Props) {
  ...
  const patch = (p: Partial<QuotationStatusFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-text-primary">Quotation Status</h2>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Validity">
          <select class={inputClass} value={props.value().validity ?? "all"} onChange={(e) => patch({ validity: e.currentTarget.value })}>
