...
import { formatDisplayDate, statusExportUrl, statusPrintPath, type QuotationStatusFilters } from "./quotationStatusFilters";
...
type Props = {
  ...
  onDateNoClick: (quotationId: number) => void;
  onProgressChange: (quotationId: number, status: string) => void;
};
...
export function QuotationStatusReport(props: Props) {
  const auth = useAuth();
  ...
  const downloadCsv = async () => {
    ...
    a.href = url;
    a.download = "quotation-status.csv";
    a.click();
  ...
  return (
    <section class="mt-6 rounded-xl border border-stroke bg-white shadow-sm">
      <div class="border-b border-stroke px-5 py-4 text-center">
        <h2 class="text-xl font-bold text-text-primary">Quotation Status</h2>
            ...
            <For each={props.rows}>
              {(row) => (
                <tr class="border-t border-stroke/60 hover:bg-slate-50/50">
                  <td class="px-3 py-2">
                    <button type="button" class="font-medium text-brand-600 hover:underline" onClick={() => props.onDateNoClick(row.quotation_id)}>
                      {row.date_no_display}
                    </button>
                  </td>
                  ...
                  <td class="px-3 py-2">
                    <ProgressStatusMenu
                      ...
                      onChange={(status) => props.onProgressChange(row.quotation_id, status)}
                    />
                  </td>
