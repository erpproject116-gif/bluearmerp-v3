...
import {
  ...
  thisMonthRange,
  todayISO,
  type OutstandingQuoteFilters,
} from "./quotationStatusFilters";

type Props = {
  value: () => OutstandingQuoteFilters;
  onChange: (next: OutstandingQuoteFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};

export function OutstandingQuoteStatusFilter(props: Props) {
  const patch = (p: Partial<OutstandingQuoteFilters>) => props.onChange({ ...props.value(), ...p });
