...
import {
  formatDisplayDate,
  outstandingExportUrl,
  poProgressStatusLabel,
  type OutstandingPOFilters,
} from "./purchaseOrderStatusFilters";
import type { PurchaseOrderOutstandingReportRow } from "../../../shared/usePurchaseOrderOutstandingReport";
...
export function OutstandingPOStatusReport(props: Props) {
  const auth = useAuth();
  ...
  const downloadCsv = async () => {
    ...
    a.href = url;
    a.download = "outstanding-po-status.csv";
    a.click();
  ...
  return (
    <section class="mt-6 rounded-xl border border-stroke bg-white shadow-sm">
      <div class="border-b border-stroke px-5 py-4 text-center">
        <h2 class="text-xl font-bold text-text-primary">Outstanding PO Status</h2>
