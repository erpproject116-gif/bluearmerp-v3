...
import { formatMoney, formatPrintDate, type Bir2307PrintPayload } from "../payment-vouchers/bir2307Print";
import "../../quotation/quotation/quotationPrint.css";

export function fetchCertificate2307Print(certificateId: number) {
  return apiFetch<Bir2307PrintPayload>(`/api/v1/finance/statutory/2307-certificates/${certificateId}/print`);
}

function Bir2307CertificatePrintView() {
  const params = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [data] = createResource(
    () => Number(params.id),
    async (id) => {
      if (!Number.isFinite(id) || id <= 0) throw new Error("Invalid certificate id.");
      const res = await fetchCertificate2307Print(id);
    ...
    <>
      ...
      <article class="quotation-print__page">
        ...
        <p class="mt-6 text-xs text-text-secondary">
          For accountant review — not a BIR e-filing submission. Verify ATC codes and TIN with your tax advisor before filing.
        </p>
        <PrintBrandingFooter />
      </article>
    </>
...
export default function Bir2307CertificatePrintPage() {
  return (
    <ProtectedRoute>
      <Bir2307CertificatePrintView />
    </ProtectedRoute>
  );
}
