...
async function fetchOpenLines(q: string, page: number, pageSize: number) {
  ...
  if (q) qs.set("q", q);
  const res = await apiFetch<OpenQuotationLineRow[]>(`/api/v1/sales-order/sales-orders/quotation-lines/open?${qs}`);
...
export function QuotationLinePickerModal(props: Props) {
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(rows.map((r) => r.quotation_line_id)));
  };
      ...
      <div class="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-slate-900/50 p-4 sm:items-center">
        <div class="w-full max-w-5xl rounded-2xl border border-stroke bg-white shadow-xl">
          <div class="flex items-center justify-between border-b border-stroke px-5 py-3">
            <h2 class="text-lg font-semibold text-text-primary">From Quotation — open lines</h2>
                      ...
                      <For each={payload().rows}>
                        {(row) => (
                          <tr
                            ...
                            onDblClick={() => toggleRow(row.quotation_line_id)}
                          >
                            <td class="py-2 pr-2">
                              <input
                                type="checkbox"
                                checked={selected().has(row.quotation_line_id)}
                                onChange={() => toggleRow(row.quotation_line_id)}
                              />
                            </td>
