export type SalesOrderStatusFilters = {
  date_from: string;
  date_to: string;
...
export type OutstandingSOFilters = SalesOrderStatusFilters & {
...
export function defaultStatusFilters(): SalesOrderStatusFilters {
  const { from, to } = thisMonthRange();
  return { date_from: from, date_to: to };
}

export function defaultOutstandingFilters(): OutstandingSOFilters {
  const { from, to } = thisMonthRange();
  return {
    date_from: from,
    date_to: to,
...
export function statusPrintPath(filters: SalesOrderStatusFilters): string {
  return `/app/sales-order/sales-orders/status/print?${filtersToSearchParams(filters).toString()}`;
}

export function statusExportUrl(filters: SalesOrderStatusFilters): string {
  return `/api/v1/sales-order/sales-orders/status-report/export?${filtersToSearchParams(filters).toString()}`;
}

export function outstandingExportUrl(filters: OutstandingSOFilters): string {
  return `/api/v1/sales-order/sales-orders/outstanding-report/export?${outstandingToSearchParams(filters).toString()}`;
}
