...
import { formatDisplayDate, statusExportUrl, statusPrintPath, type SalesOrderStatusFilters } from "./salesOrderStatusFilters";
...
type Props = {
  ...
  rows: SalesOrderStatusReportRow[];
  totalQty: number;
  totalAmount: number;
  totalRows: number;
  page: number;
  ...
  onDateNoClick: (salesOrderId: number) => void;
  onProgressChange: (salesOrderId: number, status: string) => void;
};



export function SalesOrderStatusReport(props: Props) {
  const auth = useAuth();
  ...
  return (
    <section class="mt-6 rounded-xl border border-stroke bg-white shadow-sm">
      <div class="border-b border-stroke px-5 py-4 text-center">
        <h2 class="text-xl font-bold text-text-primary">Sales Order Status</h2>
            ...
            <For each={props.rows}>
              {(row) => (
                <tr class="border-t border-stroke/60 hover:bg-slate-50/50">
                  <td class="px-3 py-2">
                    <button type="button" class="font-medium text-brand-600 hover:underline" onClick={() => props.onDateNoClick(row.sales_order_id)}>
                      {row.date_no_display}
                    </button>
                  </td>
                  ...
                  <td class="px-3 py-2">
                    <ProgressStatusMenu
                      ...
                      onChange={(status) => props.onProgressChange(row.sales_order_id, status)}
                    />
                  </td>
