...
async function fetchSlips(id: number) {
  const res = await apiFetch<CreatedSlipLine[]>(`/api/v1/sales-order/sales-orders/${id}/created-slips`);
...
export function CreatedSlipModal(props: Props) {
  const [data] = createResource(
    () => (props.open && props.salesOrderId ? props.salesOrderId : null),
                                ...
                                <For each={ln.slips}>
                                  {(s) => (
                                    <li>
                                      ...
                                      {s.slip_ref ? ` · ${s.slip_ref}` : ""}
                                      {s.release_date ? ` · ${s.release_date}` : ""} — qty {s.qty}
                                    </li>
                                  )}
                                </For>
                              </ul>
