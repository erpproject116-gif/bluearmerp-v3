...
import {
  pendingShipmentExportUrl,
  usePendingShipmentReport,
  ...
  return (
    <SalesOrderLayout>
      <ReportPageLayout
        ...
        description="Sales order lines with quantity not yet shipped — Search (F8)."
        ...
        onExportCsv={() => void downloadReportCsv(pendingShipmentExportUrl(filters()), "pending-shipment.csv")}
      >
            ...
            <For each={report.data?.rows ?? []}>
              {(row) => (
                <tr class="border-t border-stroke/60">
                  ...
                  <td class="px-3 py-2 text-right">{row.shipped_qty}</td>
                  <td class="px-3 py-2 text-right">{row.pending_qty}</td>
        ...
        <Show when={submitted() && (report.data?.rows?.length ?? 0) === 0 && !report.isFetching}>
          <p class="px-5 py-8 text-center text-sm text-text-secondary">No pending shipment lines in this date range.</p>
        </Show>
      </ReportPageLayout>
    </SalesOrderLayout>
  );
