...
import {
  shippingOrderStatusExportUrl,
  useShippingOrderStatusReport,
  type ShippingReportFilters,
} from "../../../shared/useShippingReports";
import { SalesOrderLayout } from "../SalesOrderLayout";

export default function ShippingOrderStatusPage() {
  ...
  return (
    <SalesOrderLayout>
      <ReportPageLayout
        ...
        description="Shipping order lines with freight — Search (F8)."
      ...
      pageSize={pageSize()} onPageSizeChange={setPageSize}
        ...
        onExportCsv={() => void downloadReportCsv(shippingOrderStatusExportUrl(filters()), "shipping-order-status.csv")}
      >
            ...
            <For each={report.data?.rows ?? []}>
              {(row) => (
                <tr class="border-t border-stroke/60">
                  ...
                  <td class="px-3 py-2 text-right">{formatPeso(row.freight_amount)}</td>
                </tr>
              )}
            </For>
          </tbody>
        ...
        <Show when={submitted() && (report.data?.rows?.length ?? 0) === 0 && !report.isFetching}>
          <p class="px-5 py-8 text-center text-sm text-text-secondary">No shipping orders in this date range.</p>
        </Show>
      </ReportPageLayout>
    </SalesOrderLayout>
  );
