...
export function SalesStatusFilter(props: Props) {
  ...
  const patch = (p: Partial<SalesStatusFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-text-primary">{props.title ?? "Sales Status"}</h2>
        <p class="text-sm text-text-secondary">{props.subtitle ?? "Details · by Line — set filters, then Search (F8)."}</p>
      </div>
