...
type Props = {
  ...
  onDateNoClick: (salesId: number) => void;
...
export function PreInvoicingStatusReport(props: Props) {
  ...
  const downloadCsv = async () => {
    ...
    const res = await fetch(preInvoicingExportUrl(props.filters), {
    ...
    a.href = url;
    a.download = "sales-pre-invoicing.csv";
    a.click();
  ...
  return (
    <section class="mt-6 rounded-xl border border-stroke bg-white shadow-sm">
      <div class="border-b border-stroke px-5 py-4 text-center">
        <h2 class="text-xl font-bold text-text-primary">Pre-invoicing Status</h2>
        ...
        <table class="erp-grid min-w-full text-left text-sm">
          <thead class="bg-brand-50 text-xs font-semibold uppercase text-brand-700">
            <tr>
              ...
              <th class="px-3 py-2">SI/DR No.</th>
            ...
            <For each={props.rows}>
              {(row) => (
                <tr class="border-t border-stroke/60 hover:bg-slate-50/50">
                  <td class="px-3 py-2">
                    <button type="button" class="font-medium text-brand-600 hover:underline" onClick={() => props.onDateNoClick(row.sales_id)}>
                      {row.date_no_display}
                    </button>
                  </td>
                  ...
                  <td class="px-3 py-2">
                    <ProgressStatusMenu
                      ...
                      onChange={(status) => props.onProgressChange(row.sales_id, status)}
                    />
                  </td>
                  ...
                  <td class="px-3 py-2">{row.si_dr_no ?? ""}</td>
