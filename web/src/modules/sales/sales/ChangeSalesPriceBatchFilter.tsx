...
import {
  ...
  thisMonthRange,
  todayISO,
  type PriceBatchFilters,
} from "./salesStatusFilters";

type Props = {
  value: () => PriceBatchFilters;
  onChange: (next: PriceBatchFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};
...
export function ChangeSalesPriceBatchFilter(props: Props) {
  ...
  const patch = (p: Partial<PriceBatchFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-text-primary">Change Sales Price-Batch</h2>
        <p class="text-sm text-text-secondary">Set filters, then Search (F8) to load lines for batch price update.</p>
      </div>
