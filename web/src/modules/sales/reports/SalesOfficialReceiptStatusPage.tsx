  ...
  return (
    <SalesLayout>
      ...
      <Show when={submittedFilters()}>
        <OfficialReceiptStatusReport filters={submittedFilters()!} rows={report.data?.rows ?? []} totalRows={report.data?.total ?? 0} page={page()} pageSize={pageSize()} onPageSizeChange={setPageSize} loading={report.isFetching} generatedAt={generatedAt} onPageChange={setPage} onOpenReceipt={setJournalReceiptId} />
      </Show>
