...
export default function CopilotWorkspacePage() {
  ...
  onMount(() => {
    if (!ui) return;
    ui.assistant.refreshAIConfig();
    void ui.assistant.refreshSessions();
    ui.assistant.setSessionsOpen(true);
    const sid = Number(searchParams.session);
    if (Number.isFinite(sid) && sid > 0) {
      void ui.assistant.loadSession(sid);
    }
    const q = String(searchParams.q ?? "").trim();
    if (q) {
      setSearchParams({ q: undefined, session: searchParams.session });
      queueMicrotask(() => ui.assistant.ask(q));
    }
  });

  createEffect(() => {
    if (!ui) return;
    ui.assistant.messages();
    ui.assistant.streamingText();
    queueMicrotask(() => {
  ...
  return (
    <Show
      when={ui}
      fallback={<p class="p-6 text-sm text-text-secondary">Copilot is unavailable outside the app shell.</p>}
    >
      <div class="flex h-[calc(100vh-3.5rem)] min-h-[28rem] flex-col bg-slate-50 md:flex-row">
            ...
            >
              {(s) => (
                <button
                  ...
                  onClick={() => void ui!.assistant.loadSession(s.id)}
                >
                  {s.title || "Untitled"}
                </button>
              )}
        ...
        <div class="flex min-w-0 flex-1 flex-col bg-white">
          <div ref={scrollEl} class="flex-1 overflow-y-auto px-4 py-4 md:px-8">
            <Show
              when={ui!.assistant.messages().length > 0}
              fallback={
                          ...
                          <For each={COPILOT_WORKFLOW_TILES.filter((t) => t.group === group)}>
                            {(tile) => (
                              <button
                                ...
                                onClick={() => ui!.assistant.ask(tile.query)}
                              >
            ...
            >
              <div class="mx-auto max-w-3xl">
                <HelpChatThread
                  messages={ui!.assistant.messages()}
                  pathname="/app/copilot"
                  onAsk={(text) => ui!.assistant.ask(text)}
                  streamingText={ui!.assistant.streamingText()}
                  busy={ui!.assistant.busy()}
                />
              </div>
            </Show>
          </div>
          ...
          <footer class="border-t border-stroke bg-white px-4 py-3 md:px-8">
            ...
            >
              ...
              <button
                ...
                disabled={ui!.assistant.busy() || !draft().trim()}
              >
                Send
              </button>
            </form>
