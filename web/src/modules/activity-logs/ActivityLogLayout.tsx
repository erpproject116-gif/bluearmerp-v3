...
const tabs = [
  { label: "Activity Logs", href: "/app/activity-logs" },
  { label: "Change Logs", href: "/app/activity-logs/changes" },
];
...
export const ActivityLogLayout: ParentComponent = (props) => {
  ...
  return (
    <div class="flex min-h-0 flex-1 flex-col">
      <div class="border-b border-stroke bg-white px-5 pt-4">
        <h1 class="text-lg font-semibold text-text-primary">Activity &amp; Change Logs</h1>
        <p class="mt-1 text-sm text-text-secondary">
          Activity logs show all events. Change logs show modifications in plain language with document references.
        </p>
        <nav class="mt-4 flex gap-1" aria-label="Log views">
          {tabs.map((tab) => (
            <A
              href={tab.href}
              class="rounded-t-lg border border-b-0 border-stroke px-4 py-2 text-sm font-medium transition"
              classList={{
                "bg-body text-brand-600": loc.pathname === tab.href,
                "bg-slate-50 text-text-secondary hover:bg-slate-100": loc.pathname !== tab.href,
              }}
            >
              {tab.label}
            </A>
