...
type Props = {
  value: () => CustomerQuotationsFilters;
  onChange: (next: CustomerQuotationsFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};
...
export function CustomerQuotationsFilter(props: Props) {
  const [itemLabel, setItemLabel] = createSignal("");
  const [customerLabel, setCustomerLabel] = createSignal("");
  const patch = (p: Partial<CustomerQuotationsFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-text-primary">Customer quotations by item</h2>
        <p class="text-sm text-text-secondary">Rank customers by quotation activity per product — Search (F8).</p>
      </div>
