    ...
    <div>
      <SpreadsheetGrid
        columns={[
          ...
          { key: "status", header: "Status" },
          {
            ...
            sortable: false,
            render: (r) => (
              <ActivityHistoryLink module="inventory" targetType="inv_department" targetId={r.id} title={`History — ${r.department_code}`} />
            ),
          },
        ]}
        rows={list.data?.rows ?? []}
      ...
      <EntityModal
        ...
        headerActions={
          <RecordHistoryButton
            variant="button"
            targetType="inv_department"
            targetId={editing()?.id}
            title={`History — ${editing()?.department_code ?? "Department"}`}
          />
        }
      >
