...
type Props = {
  ...
  onChange: (next: RepairOrderStatusFilters) => void;
  onSearch: () => void;
  onReset: () => void;
};
...
export function RepairOrderStatusFilter(props: Props) {
  ...
  const patch = (p: Partial<RepairOrderStatusFilters>) => props.onChange({ ...props.value(), ...p });
  ...
  return (
    <section class="rounded-xl border border-stroke bg-white p-5 shadow-sm">
      <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 class="text-lg font-semibold text-text-primary">Repair Order Status</h2>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <LookupCombo
          label="Project"
          value={projectLabel}
          selectedId={() => props.value().project_id ?? null}
          onInput={setProjectLabel}
          onSelect={(o) => {
            patch({ project_id: o.id });
        ...
        <LookupCombo
          label="PIC"
          value={picLabel}
          selectedId={() => props.value().pic_user_id ?? null}
          onInput={setPicLabel}
          onSelect={(o) => {
            patch({ pic_user_id: o.id });
            setPicLabel(o.label);
          }}
          onClear={() => {
            patch({ pic_user_id: null });
            setPicLabel("");
          }}
        ...
        <Field label="Progress status">
          <select
            class={inputClass}
            value={props.value().progress_status ?? ""}
      ...
      <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-stroke pt-4">
        ...
        <div class="flex flex-wrap gap-2 text-sm">
          <button type="button" class="text-brand-600 hover:underline" onClick={() => patch({ date_from: todayISO(), date_to: todayISO() })}>
            Today
          </button>
          <button
            type="button"
            ...
            onClick={() => {
              ...
              patch({ date_from: from, date_to: to });
            }}
          >
            This Month (~ Today)
          </button>
