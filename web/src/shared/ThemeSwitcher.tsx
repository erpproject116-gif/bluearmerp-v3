...
/** Light / Dark / System control for the shell header. */
export function ThemeSwitcher() {
  ...
  return (
    <div class="relative" ref={root}>
      <button
        type="button"
        class="rounded-lg border border-stroke p-2 text-text-secondary transition hover:erp-panel hover:text-text-primary"
        aria-label="Theme"
        aria-expanded={open()}
        title="Theme"
        onClick={(e) => {
      ...
      <Show when={open()}>
        <div class="absolute right-0 z-50 mt-1 min-w-[8rem] rounded-lg border border-stroke bg-surface py-1 shadow-lg">
          <For each={OPTIONS}>
            {(opt) => (
              <button
                type="button"
