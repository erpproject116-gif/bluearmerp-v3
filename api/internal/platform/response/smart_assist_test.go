...
func TestAssistFromMessageKnownCodes(t *testing.T) {
	cases := []struct {
		...
		{"Serial numbers required to post stock. Scan serials matching qty, then save.", SASISerialLotNeedsGR},
		...
		{"Lot numbers required to post stock. Enter lot lines matching qty, then save.", SASISerialLotNeedsGR},
		{"Serial count (1) must equal qty (2). Add the missing serials under Purchase Receive or on this bill", SASISerialLotNeedsGR},
		{"Serial count (1) must equal qty (2). Add the missing serials under Purchase Receive", SASISerialLotNeedsGR},
