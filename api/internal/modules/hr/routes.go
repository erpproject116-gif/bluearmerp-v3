...
func RegisterRoutes(r chi.Router, pool *pgxpool.Pool) {
	r.Route("/hr", func(hr chi.Router) {
		...
		hr.Group(func(admin chi.Router) {
			...
			registerAbsenteeismRoutes(admin, pool)
			...
			registerLeaveRoutes(admin, pool)
			registerDisciplineRoutes(admin, pool)
			registerHireOnboardingRoutes(admin, pool)
			registerPerformanceRoutes(admin, pool)
			registerLearningRoutes(admin, pool)
			registerPayrollDepthRoutes(admin, pool)
		})
	})
}
