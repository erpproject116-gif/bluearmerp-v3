...
func outstandingFromClause() string {
	return `
		...
		left join (
		  select sales_order_line_id, sum(release_qty) as qty_released
		...
		) tot on tot.tenant_id = so.tenant_id and tot.item_id = ln.item_id`
}

func buildOutstandingWhere(f outstandingFilters, tenantID int64) (string, []any) {
	where, args := buildSalesOrderStatusWhere(f.salesOrderStatusFilters, tenantID)
	argN := len(args) + 1
	where += fmt.Sprintf(` and (ln.qty - coalesce(rel.qty_released, 0)) > $%d`, argN)
	args = append(args, f.MinBalanceQty)
	if f.RequireStock {
		if f.StockBasis == "total" {
			where += ` and (coalesce(i.track_inventory_qty, false) = false
				or coalesce(tot.total_qty, 0) >= (ln.qty - coalesce(rel.qty_released, 0)))`
		} else {
			where += ` and (coalesce(i.track_inventory_qty, false) = false
				or coalesce(bal.qty_on_hand, 0) >= (ln.qty - coalesce(rel.qty_released, 0)))`
		}
	}
	return where, args
}
...
func outstandingOrderBy(sort, order string) string {
	allowed := map[string]string{
		...
		"balance_qty":    "(ln.qty - coalesce(rel.qty_released, 0))",
...
func queryOutstandingRows(ctx context.Context, pool *pgxpool.Pool, tenantID int64, f outstandingFilters, sort, order string, limit, offset int) ([]outstandingRow, int64, error) {
	...
	q := fmt.Sprintf(`
		select so.id, ln.id, so.order_date, so.date_seq, so.sales_order_no, so.progress_status,
		  ...
		  ln.qty::float8,
		  (ln.qty - coalesce(rel.qty_released, 0))::float8,
		  coalesce(bal.qty_on_hand, 0)::float8,
		  coalesce(tot.total_qty, 0)::float8,
		  so.delivery_date, ln.line_total::float8,
		  count(*) over()
		%s
		where %s
		order by %s
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&deliveryDate, &row.LineTotal, &total,
		); err != nil {
			return nil, 0, err
		}
...
func queryOutstandingSummary(ctx context.Context, pool *pgxpool.Pool, tenantID int64, f outstandingFilters) (outstandingSummary, error) {
	where, args := buildOutstandingWhere(f, tenantID)
	q := fmt.Sprintf(`
		select coalesce(sum(ln.qty - coalesce(rel.qty_released, 0)), 0)::float8,
...
func listSalesOrderOutstandingReport(pool *pgxpool.Pool) http.HandlerFunc {
...
func exportSalesOrderOutstandingReport(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Disposition", `attachment; filename="sales-order-outstanding.csv"`)
		cw := csv.NewWriter(w)
		_ = cw.Write([]string{
			"Date-No", "Sales Order No", "Progress Status", "Customer Name", "Location Name",
			"Item Code", "Item Name", "Qty", "Balance Qty", "Location Stock", "Total Stock",
			"Delivery Date", "Line Total",
		})
