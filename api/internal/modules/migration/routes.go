...
func RegisterRoutes(r chi.Router, pool *pgxpool.Pool) {
	r.Route("/migration", func(mr chi.Router) {
		...
		mr.Get("/opening-lots/import-template", importTemplateHandler("opening_lots"))
		mr.Get("/boms/import-template", importTemplateHandler("boms"))
		...
		mr.With(auth.RequirePermission("migration.center", auth.AccessWrite)).Post("/opening-lots/import-mapped", importOpeningLotsMapped(pool))
		mr.With(auth.RequirePermission("migration.center", auth.AccessWrite)).Post("/opening-lots/preview-mapped", previewOpeningLotsMapped(pool))
		mr.With(auth.RequirePermission("migration.center", auth.AccessWrite)).Post("/boms/import-mapped", importBomsMapped(pool))
		mr.With(auth.RequirePermission("migration.center", auth.AccessWrite)).Post("/boms/preview-mapped", previewBomsMapped(pool))
