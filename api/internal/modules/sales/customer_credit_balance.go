...
type customerCreditBalanceRow struct {
	...
	CreditOnAccount  float64  `json:"credit_on_account"`
...
func listCustomerCreditBalance(pool *pgxpool.Pool) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		q := fmt.Sprintf(`
			select partner_id, customer_name, credit_limit, credit_on_hold, open_ar_balance, credit_on_account, available_credit, total_count
			from (
			  select p.id as partner_id,
			    ...
			    coalesce(coa.credit_on_account, 0)::float8 as credit_on_account,
			    case when p.credit_limit is not null
			      then (p.credit_limit - coalesce(ar.open_balance, 0) + coalesce(coa.credit_on_account, 0))::float8
			  ...
			  left join lateral (
			    select (
			      coalesce((
			        select sum(cn.remaining_amount)
			        from public.fin_credit_notes cn
			        where cn.tenant_id = p.tenant_id and cn.partner_id = p.id and cn.deleted_at is null
			          and cn.status in ('open', 'applied') and cn.remaining_amount > 0
			      ), 0)
			      + coalesce((
			        select sum(ri.remaining_amount)
			  ...
			  where p.tenant_id = $1 and p.deleted_at is null
			    ...
			    and (p.credit_limit is not null or coalesce(ar.open_balance, 0) > 0.0001
			         or coalesce(coa.credit_on_account, 0) > 0.0001)
			) ranked
			order by %s %s
		...
		for rows.Next() {
			...
			if err := rows.Scan(&row.PartnerID, &row.CustomerName, &row.CreditLimit, &row.CreditOnHold,
				&row.OpenARBalance, &row.CreditOnAccount, &row.AvailableCredit, &totalCount); err != nil {
