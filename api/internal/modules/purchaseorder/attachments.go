...
func registerAttachmentRoutes(r chi.Router, pool *pgxpool.Pool) {
	r.Post("/purchase-orders/{id}/attachments", uploadPurchaseOrderAttachment(pool))
	r.Get("/purchase-orders/{id}/attachments", listPurchaseOrderAttachments(pool))
	r.Get("/purchase-orders/{id}/attachments/{attachmentId}/download", downloadPurchaseOrderAttachment(pool))
}

func uploadDir() string {
	return attachmentx.Dir("purchase_order")
}

func uploadPurchaseOrderAttachment(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := pool.QueryRow(r.Context(),
			`select exists(select 1 from public.po_purchase_orders where id = $1 and tenant_id = $2 and deleted_at is null)`,
			poID, tu.TenantID).Scan(&exists); err != nil || !exists {
		...
		relDir := filepath.Join(strconv.FormatInt(tu.TenantID, 10), strconv.FormatInt(poID, 10))
		...
		err = pool.QueryRow(r.Context(), `
			insert into public.po_purchase_order_attachments
			  (purchase_order_id, file_name, mime_type, size_bytes, storage_path, uploaded_by_user_id, file_bytes)
			values ($1,$2,$3,$4,$5,$6,$7)
			returning id, created_at`,
			poID, safeName, nullIfEmpty(mimeType), written, storagePath, tu.AppUserID, data).
			Scan(&id, &createdAt)
		if err != nil {
		...
		_ = audit.Log(r.Context(), pool, tu.TenantID, tu.AppUserID, "purchase_order.attachment.upload", "po_purchase_order_attachment", &id, nil, map[string]any{
			"purchase_order_id": poID,
			"file_name":         safeName,
			"size_bytes":        written,
		})

		response.OK(w, PurchaseOrderAttachment{
			ID:               id,
			FileName:         safeName,
			MimeType:         mimeType,
			SizeBytes:        written,
...
func listPurchaseOrderAttachments(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		tu, _ := auth.FromContext(r.Context())
		poID, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
		if err != nil {
		...
		rows, err := pool.Query(r.Context(), `
			...
			from public.po_purchase_order_attachments a
			join public.po_purchase_orders po on po.id = a.purchase_order_id
			where a.purchase_order_id = $1 and po.tenant_id = $2
			order by a.created_at desc`, poID, tu.TenantID)
		if err != nil {
		...
		for rows.Next() {
			var row PurchaseOrderAttachment
			var createdAt time.Time
		...
		if out == nil {
			out = []PurchaseOrderAttachment{}
		}
		response.OK(w, out, "OK")
	}
}

func downloadPurchaseOrderAttachment(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		tu, _ := auth.FromContext(r.Context())
		poID, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
		if err != nil {
		...
		err = pool.QueryRow(r.Context(), `
			...
			from public.po_purchase_order_attachments a
			join public.po_purchase_orders po on po.id = a.purchase_order_id
			where a.id = $1 and a.purchase_order_id = $2 and po.tenant_id = $3`,
			attachmentID, poID, tu.TenantID).
