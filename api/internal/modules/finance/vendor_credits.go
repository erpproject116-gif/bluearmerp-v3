...
func loadVendorCredit(ctx context.Context, q pgxQueryable, tenantID, id int64) (vendorCreditRow, error) {
	...
	err := q.QueryRow(ctx, `
		...
		select c.id, c.credit_date::text, c.credit_no, c.partner_id, c.vendor_name, c.source_supplier_invoice_id,
		  ...
		  c.refunded_at::text, c.refund_method, c.refund_reference, c.refund_official_receipt_id
		...
		&row.RefundedAt, &row.RefundMethod, &row.RefundReference, &row.RefundOfficialReceiptID,
...
func listVendorCredits(pool *pgxpool.Pool) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			response.Err(w, http.StatusInternalServerError, "Failed to list vendor credits. Apply migration 223 if needed.", "ERR_INTERNAL")
		...
		for rows.Next() {
			...
			if err := rows.Scan(&row.ID, &row.CreditDate, &row.CreditNo, &row.PartnerID, &row.VendorName, &row.SourceSupplierInvoiceID,
				...
				&row.RefundedAt, &row.RefundMethod, &row.RefundReference, &row.RefundOfficialReceiptID); err != nil {
...
func postVendorCredit(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := postVendorCreditJournal(r.Context(), tx, tu.TenantID, tu.AppUserID, id); err != nil {
			response.Err(w, http.StatusInternalServerError, "Vendor credit opened but journal failed: "+err.Error(), "ERR_INTERNAL")
			return
		}
		if err := tx.Commit(r.Context()); err != nil {
