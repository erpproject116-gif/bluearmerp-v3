...
func registerCreditNoteRoutes(r chi.Router, pool *pgxpool.Pool) {
	...
	r.With(auth.RequirePermission("finance.credit_notes_write", auth.AccessWrite)).Post("/credit-notes/backfill-journals", backfillCreditNoteJournals(pool))
...
func loadCreditNote(ctx context.Context, q pgxQueryable, tenantID, id int64) (creditNoteRow, error) {
	...
	err := q.QueryRow(ctx, `
		select c.id, c.credit_date::text, c.credit_no, c.partner_id, c.customer_name, c.source_sales_id, c.source_sales_return_id,
		  ...
		  c.refunded_at::text, c.refund_method, c.refund_reference, c.refund_payment_voucher_id, c.journal_entry_id
		...
		&row.RefundedAt, &row.RefundMethod, &row.RefundReference, &row.RefundPaymentVoucherID, &row.JournalEntryID,
	)
	if err != nil {
...
func listCreditNotes(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Err(w, http.StatusInternalServerError, "Failed to list credit notes. Apply migration 249 if needed.", "ERR_INTERNAL")
			return
		}
		defer rows.Close()
		out := []creditNoteRow{}
		for rows.Next() {
			...
			if err := rows.Scan(&row.ID, &row.CreditDate, &row.CreditNo, &row.PartnerID, &row.CustomerName, &row.SourceSalesID, &row.SourceSalesReturnID,
				&row.AmountTotal, &row.RemainingAmount, &row.Status, &row.Reason, &row.Notes,
				&row.RefundedAt, &row.RefundMethod, &row.RefundReference, &row.RefundPaymentVoucherID, &row.JournalEntryID); err != nil {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if status == "open" {
			if err := postCustomerCreditNoteJournal(r.Context(), tx, tu.TenantID, tu.AppUserID, id); err != nil {
				response.Err(w, http.StatusBadRequest, "Credit note journal failed: "+err.Error(), "ERR_BAD_REQUEST")
				return
			}
		}
...
func backfillCreditNoteJournals(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		tu, _ := auth.FromContext(r.Context())
		...
		if err != nil {
			response.Err(w, http.StatusInternalServerError, "Failed to list credits needing journals.", "ERR_INTERNAL")
			return
		}
		defer rows.Close()
		var ids []int64
		for rows.Next() {
			...
			if err := rows.Scan(&id); err != nil {
				response.Err(w, http.StatusInternalServerError, "Failed to read credit ids.", "ERR_INTERNAL")
				return
			}
			ids = append(ids, id)
		}
		...
		response.OK(w, map[string]any{"posted": posted, "scanned": len(ids), "failures": failures}, "Credit note journal backfill complete.")
	}
}
