...
func GroundedCompose(ctx context.Context, cfg Config, query, pathname string, hits []ComposeHit, pers *ComposePersonalization) (string, []string, llm.Usage, string, error) {
	return groundedCompose(ctx, cfg, query, pathname, hits, pers)
}
...
func PersistSession(ctx context.Context, pool *pgxpool.Pool, tu auth.TenantUser, sessionID *int64, pathname, query, answer string, articleIDs []string, model string, usage llm.Usage) *int64 {
	return persistComposeSession(ctx, pool, tu.TenantID, sessionID, pathname, query, answer, articleIDs, model, usage)
}
