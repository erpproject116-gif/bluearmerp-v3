...
func dashboardSummaryHandler(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_ = pool.QueryRow(ctx, `
			...
			tu.TenantID, today).Scan(&s.ExpiredQuotationsCount)

		_ = pool.QueryRow(ctx, `
			select count(*) from public.quo_quotations q
			where q.tenant_id = $1 and q.deleted_at is null
			  and q.valid_until is not null
			  and q.valid_until >= $2::date and q.valid_until <= ($2::date + interval '7 days')::date`,
			tu.TenantID, today).Scan(&s.QuotesExpiring7d)

		_ = pool.QueryRow(ctx, `
			select count(*) from public.quo_quotations q
			where q.tenant_id = $1 and q.deleted_at is null
			  and q.voucher_status = 'none'`,
			tu.TenantID).Scan(&s.QuotesNotConvertedToSO)
		...
		_ = pool.QueryRow(ctx, `
			...
			tu.TenantID).Scan(&s.WarrantyFollowUpsDue)
		...
		_ = pool.QueryRow(ctx, `
			...
			tu.TenantID).Scan(&s.QuotesMissingValidityCount)
