...
func quotationPipeline(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := pool.Query(r.Context(), `
			select q.id, q.reference_no, p.company_name, q.grand_total::float8,
			  q.valid_until, q.progress_status, q.voucher_status,
			  case
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				&card.QuotationID, &card.ReferenceNo, &card.CustomerName, &card.GrandTotal,
				&validUntil, &card.ProgressStatus, &card.VoucherStatus, &card.PipelineStage,
			); err != nil {
