...
files = [
    "api/internal/modules/manufacturing/boms.go",
    "api/internal/modules/manufacturing/reports.go",
    "api/internal/modules/manufacturing/work_orders.go",
]
...
for f in files:
    ...
    body = make_runner([upload_phase(f)], "result = {'ok': True, 'file': '" + f + "', 'outputs': outputs, 'last': outputs[-1]}")
    path = out_dir / f"_deploy_production_split_{slug}.py"
    path.write_text(body, encoding="utf-8", newline="\n")
    print("Wrote", path.name, "chars", path.stat().st_size)

rebuild_body = make_runner(
    [rebuild_phase],
...
rebuild_path = out_dir / "_deploy_production_split_rebuild.py"
rebuild_path.write_text(rebuild_body, encoding="utf-8", newline="\n")
print("Wrote", rebuild_path.name, "chars", rebuild_path.stat().st_size)
