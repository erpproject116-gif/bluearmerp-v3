...
for _ in range(200):
    ...
    if st in ('Success', 'Failed', 'PartialFailed', 'Stopped'):
        out = item
        break
result = {{'name': {name!r}, 'invoke_id': invoke_id, 'status': None if out is None else out.get('InvocationStatus'), 'exit': None if out is None else out.get('ExitCode'), 'output': None if out is None else (out.get('Output') or '')[-4000:]}}
"""
outp = patch / f"_rs_{name}.py"
outp.write_text(script, encoding="utf-8", newline="\n")
print(outp, len(script))
