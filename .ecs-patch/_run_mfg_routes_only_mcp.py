...
TOKEN_PATH = pathlib.Path(r"C:\Users\John Ranel\.mcp-auth\mcp-remote-0.1.38\47f185b1cffefbf96aa243d9a80c53f4_tokens.json")
SCRIPT_PATH = pathlib.Path(r"c:\Users\John Ranel\Documents\BluearmERP\bluearmerp-v3-github\bluearmerp-v3\.ecs-patch\_deploy_mfg_routes_only.py")
...
print("script_len", len(script), flush=True)

session, init = mcp_call(access, "initialize", {"protocolVersion": "2024-11-05", "capabilities": {}, "clientInfo": {"name": "deploy-mfg-routes", "version": "1"}}, req_id=1)
...
print("RUN_INITIAL", json.dumps({k: parsed.get(k) for k in ("status","nextAction","processID")}, ensure_ascii=False), flush=True)
...
if process_id and (parsed.get("nextAction") == "GetTask" or parsed.get("status") in ("Running", "Pending", None)):
    for i in range(200):
        ...
        print(f"POLL {i+1} status={st} nextAction={na}", flush=True)
        if st in ("Success", "Failed", "Expired", "PartialFailed", "Succeeded") and na in ("None", "Stop", None):
            break
        if na == "Stop":
            break

res = final.get("result") if isinstance(final.get("result"), dict) else final
# deploy script sets result dict; may be nested under result
if isinstance(res, dict) and "ok" in res:
    out = res
elif isinstance(final, dict) and "ok" in final:
    out = final
else:
  inner = final.get("result") or {}
...
if isinstance(last, dict):
    tail = (last.get("output") or "")[-4000:]
elif isinstance(out.get("raw_final"), dict):
    tail = json.dumps(out["raw_final"], ensure_ascii=False)[:4000]

summary = {
    "ok": out.get("ok"),
    "health_code": out.get("health_code"),
    "report_code": out.get("report_code"),
    "last_output_tail": tail,
    "deploy_last_status": last.get("status") if isinstance(last, dict) else None,
    "mcp_final_status": final.get("status"),
}
