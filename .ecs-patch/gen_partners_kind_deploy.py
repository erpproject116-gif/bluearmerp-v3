...
rel = "api/internal/modules/inventory/partners.go"
...
finish_body = "\n".join(
    [
        ...
        "mkdir -p api/internal/modules/inventory",
        f"printf '%s' '{b64}' | base64 -d > {rel}",
        "grep -n 'kind :=' api/internal/modules/inventory/partners.go | head -3",
...
# Escape triple quotes in finish body for embedding
finish_escaped = finish_body.replace("\\", "\\\\").replace('"""', '\\"""')
...
finish = \"\"\"{finish_escaped}\"\"\"
last = await run_shell(finish, timeout=900)
result = {{'ok': last.get('status')=='Success' and 'DEPLOY_PARTNERS_KIND_DONE' in (last.get('output') or ''), 'last': last}}
'''

out = pathlib.Path(__file__).with_name("_deploy_partners_kind.py")
