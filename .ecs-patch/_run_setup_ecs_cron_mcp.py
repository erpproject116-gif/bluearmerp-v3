...
SCRIPT = BASE / "_setup_ecs_cron.py"
OUT_PATH = BASE / "_setup_ecs_cron_result.json"
...
session, _ = mcp_call(access, "initialize", {"protocolVersion": "2024-11-05", "capabilities": {}, "clientInfo": {"name": "ecs-cron-setup", "version": "1"}}, req_id=1)
session, _ = mcp_call(access, "notifications/initialized", {}, req_id=2, session=session)

script = SCRIPT.read_text(encoding="utf-8")
session, run = mcp_call(access, "tools/call", {"name": "AlibabaCloud___RunScript", "arguments": {"script": script}}, req_id=3, session=session, timeout=360)
...
if process_id and not is_terminal(parsed.get("status")):
    for i in range(120):
        time.sleep(10)
        session, task = mcp_call(access, "tools/call", {"name": "AlibabaCloud___GetTask", "arguments": {"processID": process_id, "waitTimeoutSeconds": 60}}, req_id=req_id, session=session, timeout=360)
        final = parse_tool_text(task)
        req_id += 1
        if is_terminal(final.get("status")):
            break

res = final.get("result") or final
OUT_PATH.write_text(json.dumps(res, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
